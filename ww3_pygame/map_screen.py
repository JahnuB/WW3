...
class MapUI:
    def __init__(self, dimens, country):
        ...
        width, height = dimens
        self.countryName = TextSprite(self.country.name, fonts.COUNTRY_TITLE_FONT, (255,255,255), (width / 2, 100), DEFAULT_FONT_SIZE + 10)
        self.populationButton = PopulationButton((width - 200, 200))
        self.populationText = TextSprite(f"{country.pop:,}", fonts.DEFAULT_TITLE, (255,255,255), (self.populationButton.rect.centerx, self.populationButton.rect.y + 50), DEFAULT_FONT_SIZE - 10)
        self.economyButton = EconomyButton((width - 200, 300))
        self.economyText = TextSprite(country.currency + " " + f"{country.treasury["cash"][0]:,.2f}", fonts.DEFAULT_TITLE, (255,255,255) , (self.economyButton.rect.centerx, self.economyButton.rect.y + 50), DEFAULT_FONT_SIZE - 10)
        ...
        self.relationsText = TextSprite("Good" if countryRelations >= 50 else "Bad", fonts.DEFAULT_TITLE, (0,255,0) if countryRelations >= 50 else (255,0,0), (self.relationsButton.rect.centerx, self.relationsButton.rect.y + 50), DEFAULT_FONT_SIZE - 10)
        self.resourcesButton = ResourcesButton((width - 200, 500))
        self.militaryButton = MilitaryButton((width - 200, 600))
        self.militaryText = TextSprite(str(sum(self.country.recruits.values())), fonts.DEFAULT_TITLE, (255,255,255), (self.militaryButton.rect.centerx, self.militaryButton.rect.y + 50), DEFAULT_FONT_SIZE - 10)
